document.addEventListener("DOMContentLoaded", () => {
  ...
  if (isScrolled) {
    ...
    logo.classList.replace("sm:h-28", "sm:h-24");
    ...
    logoContainer.classList.remove(
      ...
      "rounded-t-2xl", "rounded-b-xl", "rounded-s-full", "rounded-e-full"
    );
    ...
    logo.classList.replace("sm:h-24", "sm:h-28");
    logo.classList.replace("lg:h-16", "lg:h-40");

    navLinks.forEach((link) => {
      link.classList.remove("text-[#C9A33A]", "hover:text-[#4B5A2A]");
      link.classList.add("text-[#C9A33A]");
    });

    // ALWAYS add pill at top
  ...
  // ✅ FIX — initialize correctly on load
